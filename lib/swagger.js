...
function parseTypedef(tags, fileComments = []) {
    ...
    for (let i = 1; i < tags.length; i++) {
        ...
        } else if (tags[i].title == 'extends') {
            const findRelatedComment = fileComments.find( comment => !!comment.tags.find( tag => {
                return tag.name === tags[i].type.name && tag.title === 'typedef';
            } ) );

            if ( findRelatedComment ) {
                const extendTags = findRelatedComment.tags.filter( tag => tag.title === 'property' );

                tags.push( ...JSON.parse( JSON.stringify( extendTags ) ) );
            }
        }
    }
...
function fileFormat(comments, fileComments = []) {
    ...
    for (let i in comments) {
        ...
        if (i == 'tags') {
            if (comments[i].length > 0 && comments[i][0]['title'] && comments[i][0]['title'] == 'typedef') {
                ...
                const typedefParsed = parseTypedef(comments[i], fileComments);
    ...
    return function (options) {
        ...
        for (let i = 0; i < apiFiles.length; i = i + 1) {
            ...
            for (let j in comments) {
                try {
                    const parsed = fileFormat(comments[j], comments);
